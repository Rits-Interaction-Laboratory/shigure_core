...
class BgSubtractionNode(NodeImagePreview):
    ...
    def __init__(self):
        ...
        self.depth_publisher = self.create_publisher(CompressedImage,
                                                     '/shigure/preview/bg_subtraction/depth/compressed', 10)
        self.avg_publisher = self.create_publisher(CompressedImage,
                                                   '/shigure/preview/bg_subtraction/avg/compressed', 10)
        self.sd_publisher = self.create_publisher(CompressedImage,
                                                  '/shigure/preview/bg_subtraction/sd/compressed', 10)
        self.result_publisher = self.create_publisher(CompressedImage,
                                                      '/shigure/preview/bg_subtraction/result/compressed', 10)
    ...
    def get_depth_callback(self, image_rect_raw: CompressedImage):
        try:
            ...
            if result:
                ...
                # self.depth_publisher.publish(self.bridge.cv2_to_compressed_imgmsg(color_depth))
                # self.avg_publisher.publish(self.bridge.cv2_to_compressed_imgmsg(color_avg))
                # self.sd_publisher.publish(self.bridge.cv2_to_compressed_imgmsg(color_sd))
                img = self.print_fps(data)
                self.result_publisher.publish(self.bridge.cv2_to_compressed_imgmsg(img))
