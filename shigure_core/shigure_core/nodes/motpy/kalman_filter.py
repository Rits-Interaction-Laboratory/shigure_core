...
# import cupy as np
# from cupy import dot, zeros, eye, isscalar, shape
...
class KalmanFilter(object):
    ...
    **Example**
    ...
    def __init__(self, dim_x, dim_z, dim_u=0):
        ...
        self.z = np.full((self.dim_z, 1), float('nan'), dtype=np.float32)
    ...
    def predict(self, u=None, B=None, F=None, Q=None):
        ...
        # Q = np.diag(np.square(np.r_[std_pos, std_vel,f_q]))
        ...
        F_shape = F.shape
        # if self.track_count % 2 == 0:
        #     self.P = np.eye(F_shape[1])
        #     self.P[0:4, 0:4] *= 100
        #     self.P[4:8, 4:8] *= 100
        #     self.P[8:, 8:] *= 1000
    ...
    def update(self, z, R=None, H=None):
        ...
        if  z[1] == []:
            ...
            self._I = np.eye(len(z[1])+8-len(z[2])*2)



            logger.info(self.x.ndim)

            feature_dmin = len(z[1])
            z_flow = z[1]
            ...
            z = np.concatenate((z[0], np.array(z[1], dtype=float)))  # z[0] は NumPy 配列, z[1] はリストとして処理
            self.dim_z = 4+feature_dmin
            ...
            if  self.H.shape[0]> 4:
                ...
                rows_to_delete = self.H.shape[0] - 4
                columns_to_delete = self.H.shape[1] - 8


                # H 行と列の削除
                self.H = self.delete_Fe(self.H, rows_to_delete, axis=0)
                self.H = self.delete_Fe(self.H, columns_to_delete, axis=1)

                # 状態ベクトル x の削除
                self.x = self.delete_Fe(self.x, rows_to_delete, axis=0)

                # 共分散行列 P の削除
                self.P = self.delete_Fe(self.P, columns_to_delete, axis=0)
                self.P = self.delete_Fe(self.P, columns_to_delete, axis=1)

                # 状態遷移行列 F の削除
                self.F = self.delete_Fe(self.F, columns_to_delete, axis=0)
                self.F = self.delete_Fe(self.F, columns_to_delete, axis=1)

                # プロセスノイズ行列 Q の削除
                self.Q = self.delete_Fe(self.Q, columns_to_delete, axis=0)
                self.Q = self.delete_Fe(self.Q, columns_to_delete, axis=1)

                # 観測ノイズ行列 R の削除
                self.R = self.delete_Fe(self.R, rows_to_delete, axis=0)
                self.R = self.delete_Fe(self.R, rows_to_delete, axis=1)
            ...
            # if  int(len(z_flow) > int(H.shape[0]-4) ):
            #     #追加される特徴の次元
            #     feature_dmin = int(len(z_flow))
            #     Pre_dmin = H.shape[0]-4
            #     add_dmin = feature_dmin - Pre_dmin 

            # if int(H.shape[0]-4) > int(len(z_flow) ) :
            #     feature_dmin = int(len(z_flow))
            #     Pre_dmin = H.shape[0]-4
            #     new_dmin = (Pre_dmin-feature_dmin )/2
            #     # print(new_dmin)

            #     new_list = list(range(int(new_dmin)))
            #     new_list =sorted(new_list ,reverse=True)
            #     lost_points = new_list
                # print("as")
                # print(lost_points)
            ...
            #logger.info(self.x)
            logger.info(self.x.shape)
            #if H.shape[1] != self.x.shape[0]:
            #    self.x = self.x[:H.shape[1]]
            #    if self.x.shape[0] < H.shape[1]:
            #        self.x = np.pad(self.x, (0, H.shape[1] - self.x.shape[0]), 'constant')
            #logger.info(H.shape)   
            #logger.info(self.x.shape)
            logger.info(z.shape[0])
            #logger.info(self.P.shape)
            ...
            for lost_point in lost_points:
                # logger.info(self.P.shape)
                # logger.info(self.F.shape)
                # logger.info(self.R.shape)
                lost_point = np.array(lost_point,dtype=int)

                row_indices = [lost_point * 2 + 5, lost_point * 2 + 4]
                col_indices = [lost_point * 2 + 9, lost_point * 2 + 8]

                if  not(col_indices[0] > self.H.shape[0]  or row_indices[0] > self.H.shape[0]):

                    # H 行と列の削除
                    self.H = self.delete_indices(self.H, col_indices, axis=1)
                    self.H = self.delete_indices(self.H, row_indices, axis=0)

                    # 状態ベクトル x の削除
                    self.x = self.delete_indices(self.x, col_indices, axis=0)

                    # 観測ベクトル z の削除
                    z = self.delete_indices(z, row_indices, axis=0)

                    # 共分散行列 P の削除
                    self.P = self.delete_indices(self.P, col_indices, axis=0)
                    self.P = self.delete_indices(self.P, col_indices, axis=1)

                    # 状態遷移行列 F の削除
                    self.F = self.delete_indices(self.F, col_indices, axis=0)
                    self.F = self.delete_indices(self.F, col_indices, axis=1)

                    # プロセスノイズ行列 Q の削除
                    self.Q = self.delete_indices(self.Q, col_indices, axis=0)
                    self.Q = self.delete_indices(self.Q, col_indices, axis=1)

                    # 観測ノイズ行列 R の削除
                    self.R = self.delete_indices(self.R, row_indices, axis=0)
                    self.R = self.delete_indices(self.R, row_indices, axis=1)
            ...
            if  int(len(z_flow) > int(H.shape[0]-4) ):
                ...
                if add_dmin > 0:
                    ...
                    self.P = self.build_P(self.P, add_dmin)
                    self.F = self.build_F(self.F, add_dmin)
                    self.H = self.build_H(self.H, add_dmin)
                    self.x = self.build_x(self.x, z_flow,self.H)
                    self.R = self.build_R(self.R, add_dmin)
                    self.Q = self.build_Q(self.Q, add_dmin)
                    


            elif  int( int(H.shape[0]- 4) > len(z_flow) ):
                feature_dmin = int(len(z_flow))
                Pre_dmin = H.shape[0]-4
                add_dmin = Pre_dmin - feature_dmin 
                if add_dmin > 0:
                    # if z[2].size == 0:
                    #     del_box = 0
                    # else:
                    #     del_box = z[2].size*2
                    # z = np.pad(z, (0, add_dmin), constant_values=0)
                    # self._I = np.eye(z[1].size+8- del_box + add_dmin)

                        # H 行と列の削除
                    self.H = self.delete_Fe(self.H, add_dmin, axis=0)
                    self.H = self.delete_Fe(self.H, add_dmin, axis=1)

                    # 状態ベクトル x の削除
                    self.x = self.delete_Fe(self.x, add_dmin, axis=0)
        
                    # 共分散行列 P の削除
                    self.P = self.delete_Fe(self.P, add_dmin, axis=0)
                    self.P = self.delete_Fe(self.P, add_dmin, axis=1)

                    # 状態遷移行列 F の削除
                    self.F = self.delete_Fe(self.F, add_dmin, axis=0)
                    self.F = self.delete_Fe(self.F, add_dmin, axis=1)

                    # プロセスノイズ行列 Q の削除
                    self.Q = self.delete_Fe(self.Q, add_dmin, axis=0)
                    self.Q = self.delete_Fe(self.Q, add_dmin, axis=1)

                    # 観測ノイズ行列 R の削除
                    self.R = self.delete_Fe(self.R, add_dmin, axis=0)
                    self.R = self.delete_Fe(self.R, add_dmin, axis=1)
                    ...
                    # self.x = self.build_x(self.x, z,self.H)
                    # self.P = self.build_P(self.P, add_dmin)
                    # self.F = self.build_F(self.F, add_dmin)
                    # self.R = self.build_R(self.R, add_dmin)
                    # self.Q = self.build_Q(self.Q, add_dmin)
            ...
            # error (residual) between measurement and prediction
            Hx = np.dot(H, self.x)
            # if z.shape[0] != Hx.shape[0]:
            #     if z.shape[0] > Hx.shape[0]:
            #         Hx = np.pad(Hx, (0, int(z.shape[0] - Hx.shape[0])), mode='constant', constant_values=0)
                
                #self.x = self.x[:z.shape[0]]
            ...
            #logger.info(self.K)
            #logger.info(self.y)
            #if self.K.shape[1] != self.y.shape[0]:
            #    if self.y.size < self.K.shape[1]:
            #        padded_array = np.pad(self.y, (0, self.K.shape[1] - self.y.size), 'constant')
            #    else:
            #        padded_array = self.y[:self.K.shape[1]]
            #    self.y  = padded_array.reshape(1, self.K.shape[1])
            #logger.info(self.y)
            ky = np.dot(self.K, self.y)
            # x = x + Ky
            # predict new x with residual scaled by the kalman gain
            #if self.x.shape[0] != ky.shape[0]:
                #ky = ky[:self.x.shape[0]]
            ...
            # and works for non-optimal K vs the equation
            # P = (I-KH)P usually seen in the literature.
            self._I = np.eye(self.K.shape[0])

            logger.info(self.K.shape)
            logger.info(self._I.shape)
            I_KH = self._I - np.dot(self.K, H)
            ...
            #特徴点追加分の次元を拡張
            # if add_dmin > 0:
            #     self.x = self.build_x(self.x,add_dmin,z_flow )
            #     self.P = self.build_P(self.P, add_dmin)
            #     self.F = self.build_F(self.F, add_dmin)
            #     self.H = self.build_H(self.H, add_dmin)
            #     self.R = self.build_R(self.R, add_dmin)
    ...
    def add_dmins(self, z) :
        print("test")
        # print(type(matrix))
        add_dmin = len(z)*2
        logger.info(self.H.shape)
        logger.info(self.x.shape)
    ...
    # 汎用的な削除関数
    def delete_indices(self, matrix, indices, axis):
        """
        指定された行列やベクトルから、指定されたインデックスを削除する。

        Parameters:
        - matrix: ndarray - 対象となる行列やベクトル
        - indices: list[int] - 削除するインデックスのリスト
        - axis: int - 削除する軸 (0: 行, 1: 列)

        Returns:
        - ndarray - 更新された行列やベクトル
        """
        # print(type(matrix))
        ...
        result = np.delete(matrix, indices, axis=axis)
    ...
    # 汎用的な削除関数
    def delete_Fe(self, matrix, indices, axis):
        """
        指定された行列やベクトルから、指定されたインデックスを削除する。
        ...
        result = np.delete(matrix, np.arange(-indices, 0), axis=axis)
    ...
    def build_P(self,P, dmin):

        matrix = np.eye(int(P.shape[0]+ dmin))
        

        matrix[P.shape[0]+ dmin:, P.shape[0]+ dmin:] *= 1000
        return matrix
    ...
    def build_F(self,F, dmin):
        """ returns constructed F matrix with specified positional
                e.g. (x,y,z) and size e.g. (w,h) dimensions """
        
        
        new_shape = (F.shape[0] + dmin, F.shape[1] + dmin)
        # 新しい配列をゼロで初期化
        new_F = np.zeros(new_shape)

        # 元の配列の値を新しい配列にコピー
        new_F[:F.shape[0], :F.shape[1]] = F

        # 拡張部分の対角成分を1で埋める
        start_row, start_col = F.shape  # 拡張部分の開始位置
        

        new_F[F.shape[0]:, F.shape[1]:] = np.eye(dmin)
    ...
    def build_H(self,H, dmin):

        new_shape = (H.shape[0] + dmin, H.shape[1] + dmin)
        # 新しい配列をゼロで初期化
        new_H = np.zeros(new_shape)

        # 元の配列の値を新しい配列にコピー
        new_H[:H.shape[0], :H.shape[1]] = H

        # 拡張部分の対角成分を1で埋める
        start_row, start_col = H.shape  # 拡張部分の開始位置

        new_H[H.shape[0]:, H.shape[1]:] = np.eye(dmin)

        # for i in range(dmin):
        #     i = i +1  
        #     new_H[start_row + i, start_col + i] = 1  # 対角成分に1を設定

        return new_H



    def build_x(self,x,flow,H):
        # if len(x) < 24:
        # z を平坦化して1次元にする
        # z_flow_repeated = np.array([item for sublist in flow for item in sublist])

        if isinstance(flow, (list, np.ndarray, np.ndarray)):  # flow がリストや配列の場合
            if np.ndim(flow) == 1:  # flow が 1 次元
                z_flow_repeated = np.array(flow)
            elif np.ndim(flow) > 1:  # flow がリストのリストや多次元配列
                print("flow",flow)
                z_flow_repeated = np.array([item for sublist in flow for item in sublist])
            else:
                raise ValueError("Unexpected flow shape or type.")
        else:
            raise TypeError("flow must be a list, numpy array, or cupy array.")

        
        # x と z_flow_repeated の次元が 48 を超えないように調整
        max_dimensions = H.shape[1]
        total_dimensions = x.size + z_flow_repeated.size

        if total_dimensions > max_dimensions:
            # 削除すべき要素数を計算
            excess = total_dimensions - max_dimensions
            z_flow_repeated = z_flow_repeated[excess:]  # 前から削る


            # flow が None の場合、空のリストとして初期化
            if flow is None:
                flow = []

    # 配列を拡張
        x = np.concatenate([x, z_flow_repeated])
    ...
    def build_R(self,R, dmin):
        

        new_shape = (R.shape[0] + dmin, R.shape[1] + dmin)
        # 新しい配列をゼロで初期化
        ...
        # 拡張部分の対角成分を1で埋める
        start_row, start_col = R.shape  # 拡張部分の開始位置

        new_R[R.shape[0]:, R.shape[1]:] = np.eye(dmin)
    ...
    def build_Q(self,Q, dmin):
        ...
        # 繰り返しで拡張部分の対角成分を生成
        repeated_values = np.resize(f_q, dmin)
        # 初期値を正方行列に拡張 (add_dmin x add_dmin)
        additional_block = np.diag(repeated_values)
        ...
        # 拡張するための新しい行列を作成
        expanded_Q = np.zeros((Q.shape[0] + dmin, Q.shape[1] + dmin))

        expanded_Q[:Q.shape[0], :Q.shape[1]] = Q  # 元の行列をコピー

        expanded_Q[Q.shape[0]:, Q.shape[1]:] = additional_block  # 拡張部分を埋める



        # motion_cov = np.diag(np.square(np.r_[std_pos]))
        # print(motion_cov)
