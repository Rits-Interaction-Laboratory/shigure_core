...
class ObjectDetectionNode(ImagePreviewNode):
    ...
    def __init__(self):
        ...
        self.time_synchronizer = message_filters.TimeSynchronizer(
            [people_subscriber, subtraction_analysis_subscriber, color_subscriber], 30000)
    ...
    def callback(self, people: People, subtraction_analysis_src: CompressedImage, color_img_src: CompressedImage):
        self.frame_count_up()

        people_mask_img = self.bridge.compressed_imgmsg_to_cv2(people.mask)
        ...
        if len(self._color_img_buffer) > 90:
            self._color_img_buffer = self._color_img_buffer[1:]
        self._color_img_buffer.append(color_img)

        top_color_image = TopColorImage(color_img_src.header.stamp.sec, color_img_src.header.stamp.nanosec,
                                        self._color_img_buffer[0])
        self.frame_object_list = self.object_detection_logic.execute(subtraction_analysis_img, people_mask_img,
                                                                     top_color_image, self.frame_object_list,
                                                                     200, 5000, 5)

        result_img = cv2.cvtColor(subtraction_analysis_img, cv2.COLOR_GRAY2BGR)

        detected_object_list = DetectedObjectList()
        detected_object_list.header = subtraction_analysis_src.header
        for frame_object in self.frame_object_list:
            if not frame_object.is_finished():
                continue
            action, bounding_box_src, size, mask_img, top_color_image = frame_object.get_item().get_items()
            x, y, width, height = bounding_box_src.get_items()
            ...
            if self.is_debug_mode:
                ...
                color_img = color_img if action == DetectedObjectActionEnum.BRING_IN else top_color_image.get_image()
                ...
                cv2.imshow(f'Result{x}{y}{width}{height}', color_img[y:y + height, x:x + width, :])
        ...
        if self.is_debug_mode:
            ...
            tile_img = cv2.vconcat([cv2.hconcat([cv2.cvtColor(subtraction_analysis_img, cv2.COLOR_GRAY2BGR),
                                                 cv2.cvtColor(people_mask_img, cv2.COLOR_GRAY2BGR)]),
                                    cv2.hconcat([cv2.cvtColor(self._known_mask * 255, cv2.COLOR_GRAY2BGR),
                                                 img])
                                    ])
